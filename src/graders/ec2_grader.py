...
def grade_episode(
    ...
    # Range [0.05, 0.95] is safe and clearly within the required interval.
    return round(float(max(0.05, min(0.95, total))), 4)
...
def grade_episode_detailed(
    ...
    return {
        ...
        "total_score": round(float(max(0.05, min(0.95, total))), 4),
    }
